...
# noinspection PyPep8Naming
def to_rdkit_mol(mol_repr, molid=None, instantiator=Chem.MolFromSmiles,
                 to2D=False, to3D=False, toPropertyMol=False, keep_Hs=True):
    """
    ...
    if mol is None:
        if molid is None:
            ...
            logger.warning('RDKit cannot create molecule %s from %r' % (molid, mol_repr))
        return mol
    if to3D:
        ...
        if -1 == AllChem.EmbedMolecule(mol, randomSeed=0):
            raise Exception('Could not embed molecule, blame stereochemistry? do not use isomeric smiles?')
...
def to_rdkit_mols(mol_reprs, instantiator=unsanitized_mol_from_smiles):
    ...
    if isinstance(mol_reprs, (str, Chem.Mol)):
        mol_reprs = [mol_reprs]
    # Generate...
    ...
    for p1, p2 in combinations(descending_number_of_atoms_order, 2):
        ...
        if p2_in_p1 and p1_in_p2:
            ...
            if num_atoms[p1] == num_atoms[p2] and not has_equal_nonequal:
                ...
                logger.info('mindblowingly, with equal number of atoms, one contains the other but not viceversa')
...
if __name__ == '__main__':
    pattern1 = 'C(=C(C#N)C(=O)NC)c1c(C)[nH]c(c)c1cc'
    pattern2 = 'c1(C=C(C#N)C(=O)NC)c(C)[nH]c2ccccc21'
    print(group_substructures([pattern1, pattern2]))
